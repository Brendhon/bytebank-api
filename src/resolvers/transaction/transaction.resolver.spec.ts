...
describe("TransactionResolver", () => {
  ...
  describe("updateTransaction Mutation", () => {
    it("should update and return the transaction", async () => {
      ...
      vi.mocked(TransactionModel.findOneAndUpdate).mockResolvedValue(
        updatedTransaction,
      );
    ...
    it("should throw an error if transaction to update is not found", async () => {
      ...
      await expect(
        ...
        `Transaction with id ${transactionId} not found or unauthorized`,
      );
    });
  });
  ...
  describe("deleteTransaction Mutation", () => {
    it("should delete the transaction and return true", async () => {
      // Arrange
      vi.mocked(TransactionModel.findOneAndDelete).mockResolvedValue(
        sampleTransaction,
      );
      const transactionId = sampleTransaction._id.toString();

      // Act
      const result = await resolver.deleteTransaction(
        transactionId,
        mockContext,
      );
    ...
    it("should throw an error if transaction to delete is not found", async () => {
      ...
      // Act & Assert
      await expect(
        resolver.deleteTransaction(transactionId, mockContext),
      ).rejects.toThrow(
        `Transaction with id ${transactionId} not found or unauthorized`,
      );
    });
