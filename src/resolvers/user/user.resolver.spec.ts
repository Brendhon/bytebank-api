...
describe("UserResolver", () => {
  ...
  describe("updateUser", () => {
    it("should update and return the user", async () => {
      ...
      const result = await userResolver.updateUser(input, {
  ...
  describe("deleteUser", () => {
    it("should delete a user and their transactions", async () => {
      ...
      vi.mocked(TransactionModel.deleteMany).mockResolvedValue({
        acknowledged: true,
        deletedCount: 5,
      } as any);
      ...
      const result = await userResolver.deleteUser({
        user: { _id: mockUser._id },
      });
  ...
  describe("validatePassword", () => {
    it("should return true for a valid password", async () => {
      ...
      const result = await userResolver.validatePassword("password123", {
        user: { _id: mockUser._id },
      });
